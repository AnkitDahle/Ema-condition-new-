    ...
    if st.session_state.current_page == "Scanner":
        ...
        if raw_data:
            ...
            if not df_selected_date.empty:
                ...
                # 3. Clean Proxy Table (Only 2 columns)
                if active_sector:
                    st.markdown(f"<h4 style='color:#ffffff; margin-top: 10px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 10px;'>📋 {active_sector} Stock List</h4>", unsafe_allow_html=True)
